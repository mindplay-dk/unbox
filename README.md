...
override(string $name, callable $func)                 # override constructor with custom function
override(string $name, callable $func, array $map)     # ... and override or add missing params
override(string $name, array $map)                     # ... or override constructor arguments
...
The following terminology is used in the documentation below:
  ...
  * **Prototype**: a function that defines how a non-singleton object (an object *not* registered
    as a component) may be generated by the factory facet of the container.
...
Note that prototypes (registered using `override()`) do not affect dependency resolution - the
life-cycle of components in a container is equal to that of the container itself, while the
life-cycle of objects created by the factory facet of the container is dependent on the scope
of the consumer; we make no attempt to define or manage e.g. a request-based, session-based, or
other type of life-cycle.
...
Note the `FactoryInterface` type-hint in the constructor - in situations where you care
only about using the container as a factory, you should type-hint against this facet.

#### Factory Overrides

The `override()` method can be used to override the behavior of the `create()` method,
effectively replacing the constructor with a custom factory function. You can also override
constructor arguments, or define a custom named prototype.
...
Overrides are useful primarily for transient non-services, such as controllers,
which get created and used by your application once, and then fall out of scope.

Note that overrides do *not* generate service registrations - override functions affect the
behavior of calls to `create()` only; the behavior of `get()` is unaffected by overrides.

To override (or define defaults) for constructor arguments:

```php
$container->override(MyController::class, ["foo" => "bar"]);

$controller = $container->create(MyController::class); // calls the controller with $foo = "bar"
```

To override a constructor completely:

```php
$container->override(MyController::class, function (...) {
    return new MyController(...)
});

$controller = $container->create(MyController::class); // calls the function defined above
```

The optional `$map` argument can be used to provide defaults for the override function - any
custom arguments given in calls to `create()` will override those defined for the custom
...
$container->override(
    ...
    ["foo" => $container->ref("some.other.component")]
);

$controller = $container->create(MyController::class); // uses "some.other.component"

$controller = $container->create(MyController::class, ["foo" => new Bar()]); // ignores "some.other.component"
```

Note that any subsequent call to `override()` will void any previous call to `override()`.
...
Non-features:
  ...
  * All registered components are singletons - we do not support prototype registrations; if you
    ...
    itself as a named component, or use the `create()` and `override()` prototype features.
...
a Windows 10 system running PHP 7.0.5.

Time to configure the container:

    pimple ........ 0.111 msec ....... 71.66% ......... 1.00x
    unbox ......... 0.117 msec ....... 75.22% ......... 1.05x
    php-di ........ 0.156 msec ...... 100.00% ......... 1.40x

Time to resolve the dependencies in the container, on first access:

    pimple ........ 0.032 msec ....... 12.10% ......... 1.00x
    unbox ......... 0.062 msec ....... 23.97% ......... 1.98x
    php-di ........ 0.260 msec ...... 100.00% ......... 8.26x

Time for multiple subsequent lookups:

    pimple: 3 repeated resolutions ........ 0.034 msec ....... 12.03% ......... 1.00x
    unbox: 3 repeated resolutions ......... 0.069 msec ....... 24.69% ......... 2.05x
    php-di: 3 repeated resolutions ........ 0.281 msec ...... 100.00% ......... 8.31x
    
    pimple: 5 repeated resolutions ........ 0.039 msec ....... 14.11% ......... 1.00x
    unbox: 5 repeated resolutions ......... 0.071 msec ....... 25.41% ......... 1.80x
    php-di: 5 repeated resolutions ........ 0.279 msec ...... 100.00% ......... 7.09x
    
    pimple: 10 repeated resolutions ........ 0.049 msec ....... 17.23% ......... 1.00x
    unbox: 10 repeated resolutions ......... 0.090 msec ....... 31.60% ......... 1.83x
    php-di: 10 repeated resolutions ........ 0.284 msec ...... 100.00% ......... 5.80x

Compared with PHP 5.6, the time needed to resolve a component is 8-10 times less under PHP 7.
